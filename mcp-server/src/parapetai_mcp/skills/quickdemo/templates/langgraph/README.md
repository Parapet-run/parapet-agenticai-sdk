# Parapet quickdemo — Sales vs HR (LangGraph / LangChain)
...
| File | What it shows |
|---|---|
| `example_no_governance.py` | Plain `langchain.agents.create_agent`, no middleware. Both people can call both tools. |
| `example_governed.py` | `ParapetAgentMiddleware`, same tools, same task. Cedar enforces org-scoped access — Tony gets Salesforce and is denied HR; Sally gets HR and is denied Salesforce. |
...
`mock_model_server.py` is a small local, OpenAI-compatible stand-in that
both examples talk to via `langchain_openai.ChatOpenAI` by default, and
both tools return canned data. Everything else is real: the real
`langchain.agents.create_agent`/`ParapetAgentMiddleware` classes, and (for
the governed example) a real Cedar policy decision from a real Parapet
control plane.

**One behavioral difference from this demo's MAF/ADK versions, worth
knowing before you read `example_governed.py`:** a denied call there
raises. `GovernedAgent`/`GovernedRunner` fold a deny into a synthetic
response and never raise; `ParapetAgentMiddleware` raises `GovernanceDenied`
before `langchain.agents.create_agent`'s own handler ever runs — a
genuine, verified block, not a soft degrade. `example_governed.py` catches
it per scenario; see that file's own module docstring.
...
3. In Claude Code (or any MCP client with this server connected), provision
   ...
   - `parapet_login`
   - `parapet_provision_agent` (display_name: "quickdemo-langgraph-governed")
...
- **total** — wall-clock latency for the whole scenario (model call + tool
  call + governance checks), timed by this demo around `agent.invoke()`,
  not something `Decision` itself carries.
- **tokens** — `total_tokens` from the model response's own usage data
  (`AIMessage.usage_metadata`, a `langchain_core` field, nothing to do
  with Cedar). `$0.00` in mock mode (canned figures from
...
(framework-neutral `Governor`, `GovernedAgent`/`GovernedRunner` in the
MAF/ADK versions of this demo, `ParapetAgentMiddleware` here, and any
future one) produces from the exact same `PolicyEngine.evaluate()` call.
Nothing about `determining_policies`/`evaluation_ms` is LangGraph-specific
— only how a DENY gets surfaced back differs per framework (an exception
here, a synthetic response in MAF/ADK — see `example_governed.py`'s own
module docstring), because that part is constrained by whatever extension
point each framework offers. The decision data itself is generic;
`example_governed.py` pulls it out of the same structlog `"decision"`
event every framework emits (`_capture` in that file), not from anything
framework-specific. `total` and `tokens` are NOT part of `Decision` at all
— this demo adds them on top from the framework's own response object, to
show governance overhead and usage side by side with the Cedar outcome.
